...
export class UIManager {
    ...
    initKitchenScene() {
        const foodItems = document.querySelectorAll('.draggable-item[data-food]');
    ...
    handleTouchDrag(source, touch, scene, onDrop) {
    ...
    initStoneStacking() {
        ...
        sourceStones.forEach(stone => {
            ...
            stone.addEventListener('touchstart', (e) => {
                ...
                const size = stone.dataset.size || 'medium';
                this.handleTouchDrag(stone, touchEvent.touches[0], 'stone', (elementUnder) => {
        ...
        const endHandler = (x, y) => {
            if (isDragging && startStar !== null) {
                ...
                if (targetStar && targetStar.id !== startStar) {
                    // Connect!
                    const alreadyConnected = this.connections.some(c => (c.from === startStar && c.to === targetStar.id) ||
    ...
    initDailyBonus() {
        ...
        if (!overlay || !closeBtn || !claimBtn || !grid)
            return;
        // Open/Close logic
