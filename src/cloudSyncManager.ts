...
export type CloudSyncEvent =
  | { type: 'status'; status: 'idle' | 'syncing' }
  | { type: 'synced'; timestamp: string }
  | { type: 'error'; message: string };

const listeners: Array<(event: CloudSyncEvent) => void> = [];

const PUSH_DELAY_MS = 2500;
let pushTimeoutId: number | null = null;
let pushing = false;

function emit(event: CloudSyncEvent): void {
...
function getSerializableState(): GameState {
  return JSON.parse(JSON.stringify(getState())) as GameState;
}

export function onCloudSyncEvent(listener: (event: CloudSyncEvent) => void): () => void {
...
export function initCloudSyncAutoPush(): void {
  subscribe(() => {
    const { cloudSync } = getState();
    if (!cloudSync.enabled || !cloudSync.recordId) {
      return;
...
export async function enableCloudSync(): Promise<{ code: string; formattedCode: string; syncedAt: string }> {
  if (!isCloudSyncConfigured()) {
...
export async function disableCloudSync(removeRemote = false): Promise<void> {
...
export async function pullCloudState(code: string): Promise<{ petName: string; coins: number; updatedAt?: string }> {
  if (!code) {
...
export function getFormattedLocalSyncCode(): string {
  const { recordId } = getState().cloudSync;
...
export async function forceCloudPush(): Promise<void> {
  await pushNow();
}
